...
<body>
	<div class="form-group form-inline" style="padding-top: 10px">
		...
		审核状态:&nbsp; <select name="status"
			class="form-control form-control-sm col-sm-1.5">
			<option value="2">请选择</option>
			<option value="0" ${article.status=="0"?"selected":""}>待审</option>
			<option value="1" ${article.status=="1"?"selected":""}>已审核</option>
		...
		<c:forEach items="${info.list}" var="article" varStatus="i">
			<tr>
				...
				<td><button type="button" class="btn btn-link"
						onclick="articelDetail(${article.id})" data-toggle="modal"
	...
	//  alert(hot);
	  ...
	  $.post("/admin/update",{id:id,hot:hot},function(flag){
		...
		function articelDetail(id){
			...
			$("#msg").empty();
			$("#content").empty();
			$.get("/admin/articelDetail",{id:id},function(article){
